...
async function serverRun(inf) {
    ...
    try {
        ...
        let infCookiesGetSet, retCookiesGetSet, results, infSendData, retSendData, infGoogleSheets, retGoogleSheets, aut, infFile, retFile, infLog, retLog, err, conSpl, leads, statusDate, statusDateFull, json, col, statusText
        let browser, page, pageValue, leadRandomNames, statusInf, chromiumHeadless, scriptHour, retClientSearch, retClientGetData, retClientImput, dataDayMonYea, dataDayMonYeaFull, dataRes, dataBoolean
        let imputRes, whileStop = false; gO.inf['stop'] = false; let tabsInf = { 'index': -1, 'name': ['INDICAR_MANUAL', 'INDICAR_AUTOMATICO', 'SOMENTE_CONSULTAR', 'LISTA_FRIA'] }; tabsInf['leadsQtd'] = tabsInf.name.map(() => 1);
        ...
        // ENCERRAR BROWSERS ANTIGOS DESSE PROJETO
        // await browsers({ 'e': e, 'project': 'C6', 'close': true, });
        await commandLine({ 'e': e, 'awaitFinish': true, 'command': `wmic Path win32_process Where "CommandLine Like '%Chrome_USER_C6%'" Call Terminate` });
        ...
        // CONSUMO DE MÉMORIA RAM (A CADA 15 MINUTOS)
        setInterval(() => {
        ...
        browser = await _puppeteer.launch({
            ...
            userDataDir: './log/Chrome_USER_C6',
            args: [ // false | 'new'
        ...
        await browsers({ 'e': e, 'project': 'C6', 'browser': browser, })
        ...
        while (!whileStop) {
            time = dateHour().res; // SEG <> DOM | [??:00] <> [??:00]
            if (!(['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SAB', 'DOM'].includes(time.dayNam) && (Number(time.hou) > Number(scriptHour[0]) - 1 && Number(time.hou) < Number(scriptHour[1])))) {
                ...
                if ((tabsInf.lastCheck[tabsInf.index]) > Math.floor(Date.now() / 1000)) {
                    ...
                    if (tabsInf.leadsQtd[tabsInf.index] == 0) {
                        ...
                        retClientSearch = await clientSearch({ 'page': page, 'leadCnpj': leadCnpj }); if (!retClientSearch.ret) {
                            logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO CLIENT SEACH` });
                            // FORÇAR PARADA DO SCRIPT
                            await browsers({ 'e': e, 'project': 'C6', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverC6/OFF.vbs`, });
                        ...
                        if (leadStatus == 'ENCONTRADO_CONTA' || leadStatus == 'ENCONTRADO_LEAD' || leadStatus == 'ENCONTRADO_EXPIRADO') {
                            ...
                            retClientGetData = await clientGetData({ 'page': page, 'leadCnpj': leadCnpj }); if (!retClientGetData.ret) {
                                logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO CLIENT GET DATA` });
                                // FORÇAR PARADA DO SCRIPT
                                await browsers({ 'e': e, 'project': 'C6', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverC6/OFF.vbs`, });
                        ...
                        if (gO.inf.sheetTab == 'SOMENTE_CONSULTAR') { statusInf = statusInf.includes(`STATUS NÃO DEFINIDO`) ? 'NADA ENCONTRADO' : leadStatus == 'ENCONTRADO_EXPIRADO' ? 'FORA DO PRAZO' : statusInf }
                            ...
                            }); if (!retClientImput.ret) {
                                logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO CLIENT IMPUT` });
                                // FORÇAR PARADA DO SCRIPT
                                await browsers({ 'e': e, 'project': 'C6', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverC6/OFF.vbs`, });
                            ...
                            } else if (imputRes == 'INDICAÇÃO OK') { statusInf = 'INDICAÇÃO OK' }
                            else if (imputRes == 'ALERTA: telefone inválido') { statusInf = gO.inf.sheetTab == 'LISTA_FRIA' ? 'DISPONÍVEL' : 'ALERTA: telefone inválido' }
                            else if (imputRes == 'ALERTA: CNPJ inválido') { statusInf = 'ALERTA: CNPJ inválido' }
                            else if (imputRes == 'ALERTA: email inválido') { statusInf = 'ALERTA: email inválido' }
                            else if (imputRes == 'ALERTA: campo não preenchido') { statusInf = 'ALERTA: campo não preenchido' }
                            else if (imputRes == 'ALERTA: status não identificado') { statusInf = 'ALERTA: status não identificado' }
                            else { statusInf = 'STATUS NÃO DEFINIDO 1' }
                        }

                        // STATUS1 [STATUS DA CONSULTA]
                        statusText = `${leadCnpj} | ${statusInf} ${statusDate}`; infSendData = { 'e': e, 'stop': false, 'status1': `${statusText}` }; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${infSendData.status1}` });
                        retSendData = await sendData(infSendData); await page.screenshot({ path: `log/screenshot_C6.jpg` }); await new Promise(resolve => { setTimeout(resolve, 1000) })
                        ...
                        infGoogleSheets = { 'e': e, 'action': 'send', 'id': gO.inf.sheetId, 'tab': gO.inf.sheetTab, 'range': `${col}${leadLinha}`, 'values': [[results]] }; retGoogleSheets = await googleSheets(infGoogleSheets);
                        if (!retGoogleSheets.ret) {
                            err = `$ [serverC6] Erro ao enviar dados para planilha`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` });
                            infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retGoogleSheets }; retLog = await log(infLog);; await page.screenshot({ path: `log/screenshot_C6_err_7.jpg` });
                            // FORÇAR PARADA DO SCRIPT
                            await browsers({ 'e': e, 'project': 'C6', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverC6/OFF.vbs`, });
                        }
                    }
