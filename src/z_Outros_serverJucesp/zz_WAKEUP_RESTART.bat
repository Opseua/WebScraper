...
rem set "timeNow=!timeNow:~0,-3!" && set "dia=!DATE:~0,2!" && set "mes=!DATE:~3,2!" && set "fileAll=log\MES_!mes!_DIA_!dia!.txt"
...
rem LER O CONFIG
set "conteudo="
for /f "usebackq delims=" %%a in ("!letra!:\ARQUIVOS\PROJETOS\Chrome_Extension\src\config.json") do (
    set "conteudo=!conteudo!%%a"
)
