...
set "outrosAdd=!outrosAdd:z_OUTROS_=%replace%!" & set "ret=NADA"
set "programExe=node" & set "fileScript=!fileProjetos!\!project!\src\!outrosAdd!"
...
endlocal & call "%fileChrome_Extension%\src\scripts\BAT\process.bat" "%arg1%_WINTP6" "%project%@%outrosAdd%" "%fileScript%" "KEEP" "%programExe%" "%ret%" & setlocal enabledelayedexpansion
set "ret=%ret2%"
rem #####################################################################
