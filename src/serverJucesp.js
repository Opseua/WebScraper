...
async function serverRun(inf) {
    ...
    try {
        ...
        let infNavigate, retNavigate, infImput, retImput, infCookiesGetSet, retCookiesGetSet, infAwaitLoad, retAwaitLoad, infCheckPage, retCheckPage, infRegex, retRegex, page
        ...
        let infButtonElement, retButtonElement, infGetTextElement, retGetTextElement, infFile, retFile, infLog, retLog, lastPage = false, err, conSpl, chromiumHeadless, token, infCommandLine, retCommandLine

        gO.inf['stop'] = false; let rate = rateLimiter({ 'max': 3, 'sec': 40 }); let time = dateHour().res; let repet1 = 999999, pg, mode, lin, range = 'A2';
        gO.inf['sheetId'] = '1h0cjCceBBbX6IlDYl7DfRa7_i1__SNC_0RUaHLho7d8'; gO.inf['sheetTab'] = 'RESULTADOS'; let infConfigStorage, retConfigStorage, browser
        ...
        infGoogleSheets = { 'e': e, 'action': 'get', 'id': gO.inf.sheetId, 'tab': gO.inf.sheetTab, 'range': range, }; retGoogleSheets = await googleSheets(infGoogleSheets); if (!retGoogleSheets.ret) {
            ...
            await browsers({ 'e': e, 'project': 'Jucesp', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverJucesp/OFF.vbs`, });
        }; gO.inf['sheetKepp'] = JSON.parse(retGoogleSheets.res[0]); aut = gO.inf.sheetKepp.aut; date = gO.inf.sheetKepp.date; mode = gO.inf.sheetKepp.mode
        conSpl = gO.inf.sheetKepp.conSpl; chromiumHeadless = gO.inf.sheetKepp.chromiumHeadless; token = gO.inf.sheetKepp.token; gO.inf['token'] = token;
        ...
        browser = await _puppeteer.launch({
            headless: chromiumHeadless, args: [ // false | 'new'
                '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-accelerated-2d-canvas', '--no-first-run', '--no-zygote', // '--single-process', '--disable-gpu', '--disable-extensions',
            ],
        }); page = await browser.newPage(); await page.setViewport({ 'width': 1024, 'height': 768 }); let pages = await browser.pages(); if (pages.length > 1) { await pages[0].close(); }

        // LOOP API
        async function loopFunRun(inf) {
            let current = `[${inf.index + 1}/${inf.length}]`  // RESULTS
            ...
            if (!retApiNire.ret) {
                ...
                if (!retApiCnpj.res || !retApiCnpj.res.cnpj) {
                    ...
                    let time = dateHour().res; retApiCnpj = retApiCnpj.res; let results = [[
                        ...
                        retApiCnpj.cnpj, retApiCnpj.telefone1,
                        retApiCnpj.telefone2, retApiCnpj.gestor1 == 'null' ? retApiCnpj.razaoSocial : retApiCnpj.gestor1, retApiCnpj.email1, retApiCnpj.razaoSocial,
                    ]]; results = results[0].join(conSpl); infSendData = { 'e': e, 'stop': false, 'results': results }; retSendData = await sendData(infSendData); ok = true
                }
            }; if (ok) {
                sheetNire.push(inf.value); infFile = { 'e': e, 'action': 'write', 'functionLocal': false, 'path': './log/NIREs.txt', 'rewrite': false, 'text': JSON.stringify(sheetNire, null, 2) }
            ...
            let indice = 0; while (!gO.inf.stop) {
                if (indice < valuesLoop.length) {
                    await loopFunRun({ 'value': valuesLoop[indice], 'index': indice, 'length': valuesLoop.length }); indice++; if (indice == valuesLoop.length && lastPage) {
                        logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `INDICES ACABARAM` }); infSendData = { 'e': e, 'stop': false, 'status2': '$ Terminou de consultar tudo' }
                        retSendData = await sendData(infSendData);
                        // FORÇAR PARADA DO SCRIPT
                        await browsers({ 'e': e, 'project': 'Jucesp', 'close': true, 'path': `${letter}:/ARQUIVOS/PROJETOS/WebScraper/src/z_Outros_serverJucesp/OFF.vbs`, });
                    }
        ...
        value = await page.content(); retCheckPage = await checkPage({ 'e': e, 'body': value, 'search': `Pesquisa Avançada` });
        if (!retCheckPage.ret) {
        ...
        infSendData = { 'e': e, 'stop': false, 'status1': 'Inserindo data de pesquisa' }; retSendData = await sendData(infSendData)
        logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${infSendData.status1}` }); await page.screenshot({ path: `log/screenshot_Jucesp.jpg` });
        ...
        if (value.includes('Digite o código da imagem')) {
            retRegex = regex({ 'e': e, 'pattern': `CaptchaImage.aspx?guid=(.*?)"`, 'text': value });
            let imagemUrl = `https://www.jucesponline.sp.gov.br/CaptchaImage.aspx?guid=${retRegex.res['1']}`; imagemUrl = JSON.stringify({ "ret": false, "msg": "COOKIE INVÁLIDO", "res": { "captcha": imagemUrl } })

            // ENVIAR URL DO CAPTCHA PARA A PLANILHA
            infGoogleSheets = { 'e': e, 'action': 'send', 'id': `1h0cjCceBBbX6IlDYl7DfRa7_i1__SNC_0RUaHLho7d8`, 'tab': `RESULTADOS`, 'range': `A50`, 'values': [[imagemUrl,]] }
            retGoogleSheets = await googleSheets(infGoogleSheets); if (!retGoogleSheets.ret) { logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO GOOGLE SHEETS` }); return retGoogleSheets }

            // STATUS1 [Aguardando texto do captcha]
            infSendData = { 'e': e, 'stop': false, 'status1': 'Aguardando texto do captcha' }; retSendData = await sendData(infSendData); logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${infSendData.status1}` })

            let delay = 1, tentativasMaximas = 60, tentativas = 0, valorCelula = '';
            while (tentativas < tentativasMaximas) {
                infGoogleSheets = { 'e': e, 'action': 'get', 'id': `1h0cjCceBBbX6IlDYl7DfRa7_i1__SNC_0RUaHLho7d8`, 'tab': `RESULTADOS`, 'range': `A50`, }; retGoogleSheets = await googleSheets(infGoogleSheets);
                if (!retGoogleSheets.ret) { logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO GOOGLE SHEETS` }); return retGoogleSheets }; valorCelula = retGoogleSheets.res[0][0];
                if (!valorCelula.includes('"ret"') && valorCelula != '') {
                    // STATUS1 [Inserindo captcha]
                    infSendData = { 'e': e, 'stop': false, 'status1': 'Inserindo captcha' }; retSendData = await sendData(infSendData); logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${infSendData.status1}` })

                    // TEXTO DO CAPTCHA INSERIDO
                    console.log(valorCelula)
                    await page.focus('input[name="ctl00$cphContent$gdvResultadoBusca$CaptchaControl1"]'); page.keyboard.type(valorCelula)

                    await new Promise(resolve => { setTimeout(resolve, 500) });

                    // BUTTON [CONTINUAR]
                    infButtonElement = { 'browser': browser, 'page': page, 'button': 'continue' }; retButtonElement = await buttonElement(infButtonElement)
                ...
                await new Promise(resolve => { setTimeout(resolve, delay * 1000) }); tentativas++;
            }
            if (tentativas >= tentativasMaximas) {
                console.log('LIMITE ALCANÇADO');
                err = `$ [serverJucesp] Cookie inválido!`
                logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` });
                infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': value }
                retLog = await log(infLog);
        ...
        // COOKIE [GET]
        infCookiesGetSet = { 'e': e, 'browser': browser, 'page': page, 'action': 'get', }; retCookiesGetSet = await cookiesGetSet(infCookiesGetSet);
        aut = retCookiesGetSet.res

        // ENVIAR NOVO COOKIE PARA A PLANILHA
        infGoogleSheets = { 'e': e, 'action': 'send', 'id': `1h0cjCceBBbX6IlDYl7DfRa7_i1__SNC_0RUaHLho7d8`, 'tab': `RESULTADOS`, 'range': `A36`, 'values': [[JSON.stringify(aut),]] }
        retGoogleSheets = await googleSheets(infGoogleSheets); if (!retGoogleSheets.ret) { logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ERRO GOOGLE SHEETS` }); return retGoogleSheets }

        // STATUS [BUSCANDO NOVOS NIRE's]
        infSendData = { 'e': e, 'stop': false, 'status1': `Buscando novos NIRE's` }; retSendData = await sendData(infSendData); logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${infSendData.status1}` })
        ...
        infFile = { 'e': e, 'action': 'read', 'functionLocal': false, 'path': './log/NIREs.txt' }; retFile = await file(infFile); try { sheetNire = JSON.parse(retFile.res) } catch (catchErr) { sheetNire = [] }
        ...
        if (mode == '←' && results[3] > 15) {
            ...
            infButtonElement = { 'browser': browser, 'page': page, 'button': 'last' }; retButtonElement = await buttonElement(infButtonElement)
            await new Promise(resolve => { setTimeout(resolve, 2000) })

            // AGUARDAR PÁGINA TERMINAR DE CARREGAR
            infAwaitLoad = { 'e': e, 'browser': browser, 'page': page, 'element': '#jo_encontrados' }; retAwaitLoad = await awaitLoad(infAwaitLoad)
            await new Promise(resolve => { setTimeout(resolve, 2000) })
            ...
            // CHECK PAGE [LISTA DE NIRE's]
            value = await page.evaluate(() => document.querySelector('*').outerHTML); retCheckPage = await checkPage({ 'e': e, 'body': value, });
            if (!retCheckPage.ret) {
                err = `$ [serverJucesp] ${retCheckPage.msg}`
                logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` });
                infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retCheckPage }
        ...
        for (let i = 0; i < repet1; i++) {
            ...
            infGetTextElement = { 'e': e, 'value': value, 'element': 'results' }
            retGetTextElement = await getTextElement(infGetTextElement); value = [retGetTextElement.res[0][0], retGetTextElement.res[0][1], retGetTextElement.res[0][2]]
            results = [pg, Number(value[0]), Number(value[1]), Number(value[2]),]; let repet2 = Number(value[1]) - Number(value[0]) + 1
            logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `MEIO\n${repet2}\n${results}` }); await page.screenshot({ path: `log/screenshot_Jucesp.jpg` });
            ...
            // STATUS [BUSCANDO NOVOS NIRE's]
            infSendData = { 'e': e, 'stop': false, 'status1': `[${mode}] NIRE's: ${results[3]} Página ${pg} de ${Math.ceil(Number(value[2]) / 15)}` }; retSendData = await sendData(infSendData)
            await new Promise(resolve => { setTimeout(resolve, 4000) })

            if (mode == '→') { pg++ } else { pg-- }
            if ((mode == '→' && Number(value[1]) == Number(value[2])) || (mode == '←' && Number(value[0]) == 1)) { logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ACABARAM AS PÁGINAS DO JUCESP` }); lastPage = true; break }
            else {
                // BUTTON [PRÓXIMA PÁGINA] / [PRÓXIMA ANTERIOR]
                if (mode == '→') { infButtonElement = { 'browser': browser, 'page': page, 'button': 'next' } } else if (mode == '←') { infButtonElement = { 'browser': browser, 'page': page, 'button': 'prev' } }

                retButtonElement = await buttonElement(infButtonElement); await new Promise(resolve => { setTimeout(resolve, 2000) })
