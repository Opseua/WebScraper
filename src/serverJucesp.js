...
async function serverRun(inf) {
    ...
    try {
        ...
        async function loopFunRun(inf) {
            ...
            if (!retApiNire.ret) {
                err = `$ [serverJucesp] FALSE: retApiNire`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` });; infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retApiNire }
                retLog = await log(infLog); let status = retApiNire.msg ? retApiNire.msg : err; await page.screenshot({ path: `log/screenshot_Jucesp_err_1.jpg` });
                ...
                if (!retApiCnpj.res || !retApiCnpj.res.cnpj) {
                    err = `$ [serverJucesp] FALSE: retApiCnpj`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` }); infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retApiCnpj }
                    retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_2.jpg` }); infSendData = { 'e': e, 'stop': true, 'status1': err }; retSendData = await sendData(infSendData)
                } else {
        ...
        value = await page.content(); retCheckPage = await checkPage({ 'e': e, 'body': value, 'search': `Pesquisa Avançada` }); if (!retCheckPage.ret) {
            ...
            retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_3.jpg` }); infSendData = { 'e': e, 'stop': true, 'status1': err }; retSendData = await sendData(infSendData);; return retCheckPage
        };
        ...
        value = await page.evaluate(() => document.querySelector('*').outerHTML); retCheckPage = await checkPage({ 'e': e, 'body': value, }); if (!retCheckPage.ret) {
            err = `$ [serverJucesp] ${retCheckPage.msg}`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` }); infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retCheckPage }
            retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_4.jpg` }); infSendData = { 'e': e, 'stop': false, 'status1': err }; retSendData = await sendData(infSendData);
            // FORÇAR PARADA DO SCRIPT
            await processForceStop()
        };
        ...
        value = await page.evaluate(() => document.querySelector('*').outerHTML); retCheckPage = await checkPage({ 'e': e, 'body': value, }); if (!retCheckPage.ret) {
            err = `$ [serverJucesp] ${retCheckPage.msg}`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` }); infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retCheckPage }
            retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_5.jpg` }); infSendData = { 'e': e, 'stop': true, 'status1': err }; retSendData = await sendData(infSendData);
            return retCheckPage
        };
        ...
        infFile = { 'e': e, 'action': 'read', 'functionLocal': false, 'path': './log/NIREs.txt' }; retFile = await file(infFile); try { sheetNire = JSON.parse(retFile.res) } catch (err) { sheetNire = [] }

        // ###################################################################################
        // GET TEXT ELEMENT [QUANTIDADE DE RESULTADOS] [PRIMEIRA PÁGINA]
        infGetTextElement = { 'e': e, 'value': value, 'element': 'results' }; retGetTextElement = await getTextElement(infGetTextElement);
        value = [retGetTextElement.res[0][0], retGetTextElement.res[0][1], retGetTextElement.res[0][2]]; pg = mode == '→' ? 1 : Math.ceil(Number(value[2]) / 15); results = [pg, Number(value[0]), Number(value[1]), Number(value[2]),]

        if (mode == '←' && results[3] > 15) {
            // BUTTON [ULTIMA PÁGINA]
            retButtonElement = await buttonElement({ 'browser': browser, 'page': page, 'button': 'last' }); await new Promise(resolve => { setTimeout(resolve, 2000) })

            // AGUARDAR PÁGINA TERMINAR DE CARREGAR
            retAwaitLoad = await awaitLoad({ 'e': e, 'browser': browser, 'page': page, 'element': '#jo_encontrados' }); await new Promise(resolve => { setTimeout(resolve, 2000) })
            ...
            value = await page.evaluate(() => document.querySelector('*').outerHTML); retCheckPage = await checkPage({ 'e': e, 'body': value, }); if (!retCheckPage.ret) {
                err = `$ [serverJucesp] ${retCheckPage.msg}`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` }); infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retCheckPage }
                retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_6.jpg` }); infSendData = { 'e': e, 'stop': true, 'status1': err }; retSendData = await sendData(infSendData);
                return retCheckPage
            };
        ...
        for (let i = 0; i < repet1; i++) {
            ...
            value = await page.evaluate(() => document.querySelector('*').outerHTML); retCheckPage = await checkPage({ 'e': e, 'body': value, }); if (!retCheckPage.ret) {
                err = `$ [serverJucesp] ${retCheckPage.msg}`; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `${err}` }); infLog = { 'e': e, 'folder': 'Registros', 'path': `${err}.txt`, 'text': retCheckPage }
                retLog = await log(infLog); await page.screenshot({ path: `log/screenshot_Jucesp_err_7.jpg` }); infSendData = { 'e': e, 'stop': true, 'status1': err }; retSendData = await sendData(infSendData); return
            };

            // GET TEXT ELEMENT [QUANTIDADE DE RESULTADOS] [DEMAIS PÁGINAS]
            retGetTextElement = await getTextElement({ 'e': e, 'value': value, 'element': 'results' }); value = [retGetTextElement.res[0][0], retGetTextElement.res[0][1], retGetTextElement.res[0][2]]
            results = [pg, Number(value[0]), Number(value[1]), Number(value[2]),]; let repet2 = Number(value[1]) - Number(value[0]) + 1; logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `MEIO\n${repet2}\n${results}` });
            await page.screenshot({ path: `log/screenshot_Jucesp.jpg` });
            ...
            // STATUS [BUSCANDO NOVOS NIRE's]
            retSendData = await sendData({ 'e': e, 'stop': false, 'status1': `[${mode}] NIRE's: ${results[3]} Página ${pg} de ${Math.ceil(Number(value[2]) / 15)}` }); await new Promise(resolve => { setTimeout(resolve, 4000) })

            if (mode == '→') { pg++ } else { pg-- }; if ((mode == '→' && Number(value[1]) == Number(value[2])) || (mode == '←' && Number(value[0]) == 1)) {
                logConsole({ 'e': e, 'ee': ee, 'write': false, 'msg': `ACABARAM AS PÁGINAS DO JUCESP` }); lastPage = true; break
            } else {
        ...
        let errMsg = `$ TRYCATCH Script erro!`; let infSendData = { 'e': e, 'stop': true, 'status1': errMsg }; let retSendData = await sendData(infSendData)
