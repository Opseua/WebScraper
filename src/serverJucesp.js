...
async function serverRun(inf = {}) {
    ...
    try {
        ...
        await navigate({ e, browser, page, 'url': 'https://www.jucesponline.sp.gov.br/BuscaAvancada.aspx?IDProduto=', }); await new Promise(resolve => { setTimeout(resolve, 1000); });
        ...
        if (retGetPath.includes('_TEMP.js')) {
            ...
            retCaptcha = await captcha({ page, 'action': 'get', }); if (retCaptcha.ret) {
                ...
                await captcha({ page, 'action': 'send', 'text': valorCelula, }); await new Promise(resolve => { setTimeout(resolve, 1000); }); await captcha({ page, 'action': 'continue', });
        ...
        if (mode === '←' && results[3] > 15) {
            ...
            await buttonElement({ browser, page, 'button': 'last', }); await new Promise(resolve => { setTimeout(resolve, 2000); });
            // AGUARDAR PÁGINA TERMINAR DE CARREGAR
            await awaitLoad({ e, browser, page, 'element': '#jo_encontrados', }); await new Promise(resolve => { setTimeout(resolve, 2000); });
