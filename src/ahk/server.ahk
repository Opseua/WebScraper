...
!z:: { ; Alt+Z
    ...
    try {
		if isRunning {
			title := 'INPUT AUTOMÁTICO', text := 'Aguarde o input em andamento!'
			; MsgBox(title '`n' text)
...
inputText := InputBox('Cole as informações aqui:', 'INPUT AUTOMÁTICO', 'w450 h90')
if (inputText.Result = 'Cancel') {
	return
} else if (inputText.Value = '' || !RegExMatch(inputText.Value, '^(?:[^\t]*\t){4}[^\t]*$')) {
	title := 'INPUT AUTOMÁTICO', text := 'ALERTA: confira o texto inserido!'
	; MsgBox(title '`n' text)
	notification(title, text, 3)
	return
} 
...
title := 'INPUT AUTOMÁTICO', text := 'Indicando...'
; MsgBox(title '`n' text)
...
Sleep(2000)
title := 'INPUT AUTOMÁTICO', text := 'Terminado!'
; MsgBox(title '`n' text)
notification(title, text, 2)
